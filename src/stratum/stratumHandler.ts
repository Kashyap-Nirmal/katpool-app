...
import { getDifficulty, getSocketLogData, processCanxiumAddress } from './utils';
...
export class StratumHandler {
  ...
  authorize(socket: Socket<Miner>, request: Request) {
    ...
    if (!canxiumAddr) this.monitoring.log(`Canxium address is not set.`);
      ...
      processedCanxiumAddress = processCanxiumAddress(canxiumAddr);
    }
    ...
    if (DEBUG)
      this.monitoring.debug(
        `StratumHandler ${this.sharesManager.port}: Authorizing worker - Address: ${address}, Worker Name: ${name}, Canxium address: ${processedCanxiumAddress}`
      );
